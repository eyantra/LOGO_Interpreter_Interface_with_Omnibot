...
NODE *make_object(NODE *canonical, NODE *oproc, NODE *val, NODE *plist,
		NODE *casestrnd) {
	NODE *temp;

	temp = cons_list(0, canonical, oproc, val, plist, make_intnode((FIXNUM) 0),
			END_OF_LIST);
...
NODE *find_case(NODE *strnd, NODE *obj) {
	...
	while (clist != NIL && case_compare(strnd, strnode__caseobj(car(clist))))
		clist = cdr(clist);
	if (clist == NIL)
		return (NIL);
	else
