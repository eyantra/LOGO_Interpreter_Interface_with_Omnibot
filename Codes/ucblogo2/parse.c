...
NODE *reader(FILE *strm, char *prompt) {
	...
	*phys_line = '\0';
	...
	ret = make_strnode(p_line, (struct string_block *) NULL, (int) strlen(
			p_line), this_type, strnzcpy);
...
NODE *parser_iterate(char **inln, char *inlimit, struct string_block *inhead,
	...
	do {
		...
		else if (vbar || (!white_space(ch) && ch != ']' && ch != '{' && ch
				!= '}' && ch != '['))
			windex++;
		...
		/* if this is a '[', parse a new list */
			...
			tnode = cons(list_to_array(parser_iterate(inln, inlimit, inhead,
					semi, '}')), NIL);
			if (**inln == '@') {
				int i = 0, sign = 1;
		...
		else if (white_space(ch) || **inln == ']' || **inln == '[' || **inln
				== '{' || **inln == '}') {
			if (windex > 0 || this_type == VBAR_STRING) {
				if (broken == FALSE)
					tnode = cons(make_strnode(wptr, inhead, windex, this_type,
							strnzcpy), NIL);
				else {
					tnode = cons(make_strnode(wptr,
							(struct string_block *) NULL, windex, this_type,
							(semi ? mend_strnzcpy : mend_nosemi)), NIL);
					broken = FALSE;
				}
				this_type = STRING;
				windex = 0;
	...
	while (wcnt < wlen) {
		...
		if (*wptr == '"') {
			...
			if (wtyp == PUNBOUND) {
				...
				tnode = cons(make_quote(intern(make_strnode(tptr, NULL, tcnt,
						wtyp, noparity_strnzcpy))), NIL);
			} else
			...
			tnode = cons(make_colon(intern(make_strnode(tptr, whead, tcnt,
					wtyp, strnzcpy))), NIL);
		} else if (wcnt == 0 && *wptr == '-' && monadic_minus == FALSE && wcnt
				+ 1 < wlen && !white_space(*(wptr+1))) {
			...
			else if (wcnt + 1 < wlen && ((*wptr == '<' && (*(wptr + 1) == '='
					|| *(wptr + 1) == '>')) || (*wptr == '>' && *(wptr + 1)
					== '='))) {
				tnode = cons(intern(make_strnode(wptr, whead, 2, STRING,
						strnzcpy)), NIL);
				wptr++, wcnt++;
			} else
				tnode = cons(intern(make_strnode(wptr, whead, 1, STRING,
						strnzcpy)), NIL);
			monadic_minus = FALSE;
			wptr++, wcnt++;
		} else {
			...
			while (wcnt < wlen && !parens(*wptr) && (!infixs(*wptr) || (isnumb
					== 1 && (*wptr == '-' || *wptr == '+')))) {
				if (isnumb == 4 && isdigit(*wptr))
					isnumb = 0;
				if (isnumb == 0 && tcnt > 0 && (*wptr == 'e' || *wptr == 'E'))
					isnumb = 1;
				else if (!(isdigit(*wptr) || (!gotdot && *wptr == '.'))
						|| isnumb == 1)
					isnumb = 2;
				if (*wptr == '.')
					gotdot++;
			...
			if (isnumb == 3 && tcnt > 1) { /* ?5 syntax */
				...
				qmtnode = cons_list(0, Left_Paren, Query,
						cnv_node_to_numnode(make_strnode(tptr + 1, whead, tcnt
								- 1, wtyp, strnzcpy)), END_OF_LIST);
				if (outline == NIL) {
					outline = qmtnode;
				} else {
				...
				tnode = cons(cnv_node_to_numnode(make_strnode(tptr, whead,
						tcnt, wtyp, strnzcpy)), NIL);
			} else
				tnode = cons(intern(make_strnode(tptr, whead, tcnt, wtyp,
						strnzcpy)), NIL);
		}
