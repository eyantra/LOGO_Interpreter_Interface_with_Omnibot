...
void draw_turtle(void) {
	...
	if ((turtle_y > turtle_top_max - turtle_height) && (current_mode
			== wrapmode)) {
	...
	if ((turtle_y < turtle_bottom_max + turtle_height) && (current_mode
			== wrapmode)) {
		turtle_y += (screen_height + 1);
		draw_turtle_helper();
		check_x_high();
...
void check_x_high(void) {
	if ((turtle_x > turtle_right_max - turtle_height) && (current_mode
			== wrapmode)) {
...
void check_x_low(void) {
	if ((turtle_x < turtle_left_max + turtle_height) && (current_mode
			== wrapmode)) {
		turtle_x += (screen_width + 1);
		draw_turtle_helper();
		turtle_x -= (screen_width + 1);
...
void draw_turtle_helper(void) {
	...
	top_x = g_round(turtle_x + x_scale * (FLONUM) (cos_real_heading
			* turtle_side));
	top_y = g_round(turtle_y + y_scale * (FLONUM) (sin_real_heading
			* turtle_side));
...
void forward_helper(FLONUM d) {
	...
	if ((dx < 0 && dx > -0.000001) || (dx > 0 && dx < 0.000001))
		dx = 0;
	if ((dy < 0 && dy > -0.000001) || (dy > 0 && dy < 0.000001))
		dy = 0;
	x2 = x1 + dx;
	y2 = y1 - dy;
	...
	if (!drawing_turtle && check_throwing)
		return;
	...
	if (current_mode == windowmode || (rx2 >= screen_left && rx2
			<= screen_right && ry2 >= screen_top && ry2 <= screen_bottom)) {
		...
		if ((newd = wrap_right(d, x1, y1, x2, y2)) != 0.0)
			goto wraploop;
		if ((newd = wrap_left(d, x1, y1, x2, y2)) != 0.0)
			goto wraploop;
		if ((newd = wrap_up(d, x1, y1, x2, y2)) != 0.0)
			goto wraploop;
		if ((newd = wrap_down(d, x1, y1, x2, y2)) != 0.0)
			goto wraploop;
	}
...
NODE *vec_arg_helper(NODE *args, BOOLEAN floatok, BOOLEAN three) {
	...
	while (NOT_THROWING) {
		if (arg != NIL && is_list(arg) && cdr(arg) != NIL
				&& (three ? (cddr(arg) != NIL && cdr(cddr(arg)) == NIL)
						: cddr(arg) == NIL)) {
			val1 = cnv_node_to_numnode(car(arg));
			val2 = cnv_node_to_numnode(cadr(arg));
			if (three)
				val3 = cnv_node_to_numnode(car(cddr(arg)));
			if (val1 != UNBOUND && val2 != UNBOUND && (floatok || (nodetype(
					val1) == INT && getint(val1) >= 0 && nodetype(val2) == INT
					&& getint(val2) >= 0 && (!three || (nodetype(val3) == INT
					&& getint(val3) >= 0))))) {
				setcar(arg, val1);
				setcar(cdr(arg), val2);
				if (three)
...
NODE *lpos(NODE *args) {
	return (cons(make_floatnode(cut_error(turtle_x / x_scale)), cons(
			make_floatnode(cut_error(turtle_y / y_scale)), NIL)));
}
...
void cs_helper(int centerp) {
	...
	if (turtle_x != 0.0 || turtle_y != 0.0)
		save_move();
	if (pen_vis != 0)
		save_vis();
	if (internal_penmode != PENMODE_PAINT)
		save_mode();
	if (pen_color != 7)
		save_color();
...
void setpos_commonpart(FLONUM target_x, FLONUM target_y) {
	...
	if (NOT_THROWING) {
		...
		if (current_mode == fencemode && wrapping)
			...
			turtle_heading = towards_helper(target_x, target_y, wanna_x,
					wanna_y);
			tx = wanna_x / x_scale;
			ty = wanna_y / y_scale;
#define sq(z) ((z)*(z))
...
void setpos_helper(NODE *xnode, NODE *ynode) {
	...
	if (NOT_THROWING) {
		...
		draw_turtle();
		move_to(g_round(screen_x_coord), g_round(screen_y_coord));
		target_x = ((xnode == NIL) ? turtle_x / x_scale : ((nodetype(xnode)
				== FLOATT) ? getfloat(xnode) : (FLONUM) getint(xnode)));
		target_y = ((ynode == NIL) ? turtle_y / y_scale : ((nodetype(ynode)
				== FLOATT) ? getfloat(ynode) : (FLONUM) getint(ynode)));
		setpos_commonpart(target_x, target_y);
		done_drawing;
	}
}
...
NODE *lturtlemode(NODE *args) {
	switch (current_mode) {
	case wrapmode:
		return (theName(Name_wrap));
	case fencemode:
		return (theName(Name_fence));
	case windowmode:
		return (theName(Name_window));
	}
...
NODE *lscreenmode(NODE *args) {
	switch (screen_mode) {
	case SCREEN_TEXT:
		return (theName(Name_textscreen));
	case SCREEN_SPLIT:
		return (theName(Name_splitscreen));
	case SCREEN_FULL:
		return (theName(Name_fullscreen));
	}
	return (UNBOUND); /* Can't get here, but makes compiler happy */
}
...
NODE *lpensize(NODE *args) {
	return (cons(make_intnode((FIXNUM) pen_width), cons(make_intnode(
			(FIXNUM) pen_height), NIL)));
}
...
NODE *lpenmode(NODE *args) {
	...
	case PENMODE_PAINT:
		return theName(Name_paint);
	case PENMODE_ERASE:
		return theName(Name_erase);
	case PENMODE_REVERSE:
		return theName(Name_reverse);
	}
	return (UNBOUND); /* Can't get here, but makes compiler happy */
}
...
NODE *lsetpalette(NODE *args) {
	...
	if (slotnum < -SPECIAL_COLORS) {
		...
		//		prepare_to_draw;
		set_palette(slotnum, (unsigned int) (get_number(arg) * 65535 / 100.0),
		...
		//		done_drawing;
		if (slotnum > max_palette_slot)
			max_palette_slot = slotnum;
	}
	return (UNBOUND);
}
...
NODE *lpalette(NODE *args) {
	...
	if (getint(arg) < -SPECIAL_COLORS)
		err_logo(BAD_DATA_UNREC, arg);
	if (NOT_THROWING) {
		get_palette((int) getint(arg), &r, &g, &b);
		return cons(make_rgbnode(r), cons(make_rgbnode(g), cons(
				make_rgbnode(b), NIL)));
	}
	return UNBOUND;
}
...
void restore_palette(FILE *fp) {
	...
	if (nslots > max_palette_slot)
		max_palette_slot = nslots;
	for (i = 8; i <= nslots; i++) {
...
NODE *lmousepos(NODE *args) {
	return (cons(make_floatnode(mouse_x / x_scale), cons(make_floatnode(mouse_y
			/ y_scale), NIL)));
}
...
void do_arc(FLONUM count, FLONUM ang, FLONUM radius, FLONUM delta, FLONUM tx,
		FLONUM ty, FLONUM angle, FLONUM thead, BOOLEAN save) {
	FLONUM x;
	FLONUM y;
	FLONUM i;
...
BOOLEAN safe_to_save(void) {
	...
	if (record_index < (GR_SIZE - 300))
		return TRUE; /* room here */
...
void save_arc(FLONUM count, FLONUM ang, FLONUM radius, FLONUM delta, FLONUM tx,
		FLONUM ty, FLONUM angle, FLONUM thead) {
	if (safe_to_save()) {
...
void redraw_graphics(void) {
	...
	while (bufp[r_index] != FINISHED) {
		...
		case (SETPENCOLOR):
			...
			break;
		case (SETPENRGB):
			set_palette(PEN_COLOR_OFFSET, (*(int *) (bufp + r_index + One)),
					(*(int *) (bufp + r_index + Two)), (*(int *) (bufp
							+ r_index + Three)));
		...
		case (FILLERUP):
			...
			break;
		case (ARC):
			do_arc(*(FLONUM *) (bufp + r_index + Big), *(FLONUM *) (bufp
					+ r_index + 2 * Big),
					*(FLONUM *) (bufp + r_index + 3 * Big), *(FLONUM *) (bufp
							+ r_index + 4 * Big), *(FLONUM *) (bufp + r_index
							+ 5 * Big), *(FLONUM *) (bufp + r_index + 6 * Big),
					*(FLONUM *) (bufp + r_index + 7 * Big), *(FLONUM *) (bufp
							+ r_index + 8 * Big), 1);
			r_index += 9 * Big;
			break;
		case (NEXTBUFFER):
	...
	if (NOT_THROWING) {
		...
		while (buf != record) {
			...
			while (want > 0) {
				...
				if (ferror(fp) || cnt <= 0) {
					err_logo(FILE_ERROR, make_static_strnode("File too big"));
					lclose(args);
					return UNBOUND;
				}
				want -= cnt;
...
NODE *lepspict(NODE *args) {
	...
	if (NOT_THROWING) {
		...
		ndprintf(fp, "%%Title: %p\n", car(args));
		fprintf(fp,
				"%%%%Creator: Logo2PS 1.1; Copyright 1997, Vladimir Batagelj\n");
		...
		while (bufp[r_index] != FINISHED) {
			...
			case (LABEL):
				...
				fprintf(fp, "%d %d moveto\n", lastx, lasty);
				r_index += (One + 2 + bufp[r_index + One] + (One - 1)) & ~(One
						- 1);
				break;
			case (SETPENVIS):
				vis = bufp[r_index + 1];
			...
			case (FILLERUP):
				...
				break;
			case (ARC): {
				FLONUM tx = screen_x_center + *(FLONUM *) (bufp + r_index + 5
						*Big);
				FLONUM ty = screen_y_center + *(FLONUM *) (bufp + r_index + 6
						*Big);
				...
				fprintf(fp, "%f %f moveto\n", tx + radius * cos(degrad * (90
						- thead - angle)), ty + radius * sin(degrad * (90
						- thead - angle)));

				/* x-coord y-coord r ang1 ang2 arc  - */
				fprintf(fp, "%f %f %f %f %f arc\n", tx, ty, radius, 90 - thead
						- angle, 90 - thead);
