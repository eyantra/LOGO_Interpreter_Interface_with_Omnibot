...
*catch_tag = NIL, *arg = NIL; /* the current actual */
...
NODE *err_eval_driver(NODE *seq, BOOLEAN recoverable) {
	val_status = (recoverable ? VALUE_OK : NO_VALUE_OK) | (val_status
			& (OUTPUT_OK | STOP_OK));
...
NODE *deep_copy(NODE *exp) {
	...
	if (exp == NIL)
		...
		for (p = getarrptr(exp), q = getarrptr(val), arridx = 0; arridx
				< getarrdim(exp); arridx++, p++)
			*q++ = deep_copy(*p);
	} else
		val = exp;
	return val;
...
NODE *evaluator(NODE *list, enum labels where) {
	...
	begin_line: this_line = list;
	val_status = NO_VALUE_OK;
	newcont(end_line);
	begin_seq: debprint("begin_seq");
	make_tree(list);
	if (!is_tree(list)) {
		val = UNBOUND;
		goto fetch_cont;
	...
	tail_eval_dispatch: tailcall = 1;
	eval_dispatch: debprint("eval_dispatch");
	switch (nodetype(exp)) {
	...
	case CONS: /* procedure application */
		if (tailcall == 1 && is_macro(car(exp))
				&& (is_list(procnode__caseobj(car(exp))) || isName(car(exp),
						Name_goto))) {
	...
	unev = cdr(exp);
	argl = NIL;
	eval_arg_loop: debprint("eval_arg_loop");
	if (unev == NIL)
		goto eval_args_done;
	exp = car(unev);
	...
	arg_from_macro: if (nodetype(exp) != CONS) { /* Don't bother saving registers */
	...
	accumulate_arg: debprint("accumulate_arg");
		...
		if (val == NIL || val == UNBOUND || cdr(val) != NIL) {
			if (NOT_THROWING) {
				if (tree_dk_how != NIL && tree_dk_how != UNBOUND)
					...
					err_logo((val != NIL && val != UNBOUND) ? RUNNABLE_ARG
							: ERR_MACRO, val);
			}
			goto eval_args_done;
		}
	...
	after_const_arg: if (stopping_flag == THROWING)
		goto eval_args_done;
	push(val, argl);
	pop(unev);
	goto eval_arg_loop;

	eval_args_done: if (stopping_flag == THROWING) {
		val = UNBOUND;
		goto fetch_cont;
	}
	argl = reverse(argl);
	/* --------------------- APPLY ---------------------------- */
	apply_dispatch: debprint("apply_dispatch");
	...
	if (proc == UNDEFINED) { /* 5.0 punctuationless variables */
		if (!varTrue(AllowGetSet)) { /* No getter/setter allowed, punt */
			...
			if (val == UNBOUND && NOT_THROWING)
				...
				(void) ldefine(cons(fun, cons(cons(NIL, cons(cons(theName(
						Name_output), cons(make_colon(fun), NIL)), NIL)), NIL))); /* make real proc so no disk load next time */
			...
			(void) ldefine(cons(fun, cons(cons(Listvalue, cons(cons(Make, cons(
					make_quote(bf3(fun)), cons(Dotsvalue, NIL))), NIL)), NIL)));
			setflag__caseobj(fun,PROC_BURIED);
...
#define do_case(x) case x: goto x;
	...
	compound_apply: debprint("compound_apply");
	...
	lambda_apply: vsp = var_stack; /* remember where we came in */
	for (formals = formals__procnode(proc); formals != NIL; formals
			= cdr(formals)) {
		parm = car(formals);
		...
		if (nodetype(parm) == CASEOBJ) {
			...
			if (arg == UNBOUND) { /* use default */
				...
				tailcall = -1;
				list = cdr(parm);
				didnt_get_output = cons_list(0, fun, ufun, list, END_OF_LIST);
				didnt_output_name = NIL;
				if (NOT_THROWING)
					make_tree(list);
				else
				...
				set_args_continue: if (stopping_flag == MACRO_RETURN) {
					if (val == NIL || val == UNBOUND || cdr(val) != NIL) {
						if (NOT_THROWING)
							err_logo(
									(val != NIL && val != UNBOUND) ? RUNNABLE_ARG
											: ERR_MACRO, val);
					} else {
						reset_args(var);
						exp = car(val);
						stopping_flag = RUN;
						didnt_get_output = cons_list(0, fun, ufun, list,
								END_OF_LIST);
						didnt_output_name = NIL;
						tailcall = -1;
						newcont(set_args_continue);
	...
	if (didnt_get_output == UNBOUND)
		...
		val_status = NO_VALUE_OK | STOP_OK | STOP_TAIL | OUTPUT_OK
				| OUTPUT_TAIL;
	else
		val_status = OUTPUT_OK | OUTPUT_TAIL;
	...
	eval_sequence: debprint("eval_sequence");
		...
		if (i == OUTPUT_PRIORITY) {
			...
			if (val_status & OUTPUT_TAIL) {
				...
				if (val_status & STOP_OK) {
					...
					goto eval_dispatch;
					after_maybeoutput: if (val == UNBOUND)
						lstop(NIL);
					else
	...
	if (unev == NIL) { /* falling off tail of sequence */
		...
		if (val_status & NO_VALUE_OK) {
			...
			next_stop_want_output: save(didnt_get_output);
			didnt_get_output = UNBOUND;
			val_status &= ~OUTPUT_TAIL;
			newcont(fall_off_want_output);
			goto tail_eval_dispatch;
			fall_off_want_output: restore(didnt_get_output);
			if (stopping_flag == OUTPUT) {
				goto fetch_cont;
				/* repeat body did output */
			}
	...
	non_tail_eval: debprint("non_tail_eval");
	...
	eval_sequence_continue: reset_args(var);
	...
	after_constant: if (val != UNBOUND && NOT_THROWING) {
		err_logo(DK_WHAT, val);
		val = UNBOUND;
	}
	...
	macro_return: restore2(didnt_get_output,current_unode);
...
#define RUNRESULT_OUTPUT_LEGAL 0
	...
	runresult_continuation: list = val;
	...
	runresult_followup: restore(current_unode);
	...
	repeat_continuation: list = cdr(val);
	num2save(repcount,user_repcount);
	repcount = getint(car(val));
	user_repcount = 0;
	repeat_again: val = UNBOUND;
	if (repcount == 0) {
		repeat_done: num2restore(repcount,user_repcount);
		goto fetch_cont;
	}
	user_repcount++;
	save2(list,var);
	...
	if (tailcall == 0)
		val_status |= NO_VALUE_OK; /* embedded repeat */
	...
	catch_continuation: list = cdr(val);
	catch_tag = car(val);
	...
	catch_followup: restore2(current_unode,catch_tag);
	num2restore(val_status,tailcall);
	...
	if (NOT_THROWING && val != UNBOUND && !(val_status & VALUE_OK))
		err_logo(DK_WHAT, val);
	if (stopping_flag == THROWING
			&& ((compare_node(throw_node, catch_tag, TRUE) == 0) || (isName(
					throw_node, Name_error) && isName(catch_tag, Name_error)))) {
		throw_node = UNBOUND;
		stopping_flag = RUN;
		val = output_node;
	}
	...
	goto_continuation: if (NOT_THROWING) {
		if (ufun == NIL) {
		...
		while (unev != NIL && !check_throwing) {
			...
			exp = car(unev);
			pop(unev);
			if (is_list (exp) && (isName(car(exp), Name_tag)) && (nodetype(
					cadr(exp)) == QUOTE) && compare_node(val,
					node__quote(cadr(exp)), TRUE) == 0) {
	...
	if (NOT_THROWING && exp != NIL) {
		if (is_list(exp)) { /* template */
			if (is_list(car(exp)) && cdr(exp) != NIL) {
				...
				/*		numrestore(tailcall);	*/
				for (; formals != NIL && argl != NIL && NOT_THROWING; formals
						= cdr(formals), argl = cdr(argl))
