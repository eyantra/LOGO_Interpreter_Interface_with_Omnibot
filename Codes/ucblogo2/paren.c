...
void check_library(NODE *first) {
	if (procnode__caseobj(first) == UNDEFINED && NOT_THROWING && first
			!= Null_Word)
...
void make_tree_from_body(NODE *body) {
	...
	if (body == NIL || (is_tree(body) && generation__tree(body)
			== the_generation))
		return;
	if (is_tree(body))
		untreeify_body(body);
...
void make_tree(NODE *list) {
	...
	if (list == NIL || (is_tree(list) && generation__tree(list)
			== the_generation))
		return;
	if (!runparsed(list))
		make_runparse(list);
...
int missing_space(NODE *name) {
	...
	err_logo(MISSING_SPACE, cons_list(0, cons_list(0, missing_alphabetic,
			missing_numeric, END_OF_LIST), name, END_OF_LIST));
	return 1;
}
	...
	if (nodetype(first) == CASEOBJ && !numberp(first)) {
		if (first == Left_Paren) {
			...
			if (*expr == NIL)
				...
				if (is_list(first))
					first = car(first);
				if (nodetype(first) != CASEOBJ || procnode__caseobj(first)
						== UNDEFINED)
...
NODE *gather_some_args(int min, int max, NODE **args, BOOLEAN inparen,
	...
	if (*args == NIL || car(*args) == Right_Paren || (nodetype(car(*args))
			== CASEOBJ && nodetype(procnode__caseobj(car(*args))) == INFIX)) {
		if (min > 0)
		...
		if (ifnode != (NODE **) NIL && is_list(car(*args))) {
			...
			*ifnode = theName(Name_ifelse);
			...
			return cons(retval, gather_some_args(min, max, args, inparen,
					(NODE **) NIL));
		}
	} else {
		if (max < 0)
			max = 0; /* negative max means unlimited */
		if (car(*args) != Right_Paren && (nodetype(car(*args)) != CASEOBJ
...
int priority(NODE *proc_obj) {
	...
	if (nodetype(proc_obj) != CASEOBJ || (pproc = procnode__caseobj(proc_obj))
			== UNDEFINED || nodetype(pproc) != INFIX)
		return 0;
...
NODE *paren_infix(NODE *left_arg, NODE **rest, int old_pri, BOOLEAN inparen) {
	...
	if (*rest == NIL || !(pri = priority(infix_proc = car(*rest))) || pri
			<= old_pri)
		return left_arg;
	pop(*rest);
