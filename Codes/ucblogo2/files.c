...
FILE *open_file(NODE *arg, char *access) {
	...
	if (arg == UNBOUND)
		return (NULL);
	if (file_prefix != NIL) {
		print_stringlen = getstrlen(file_prefix) + getstrlen(arg) + 2;
...
FILE *find_file(NODE *arg, BOOLEAN remove) {
	...
	t = file_list;
	while (t != NIL) {
		if ((is_list(arg) && arg == car(t)) || (!is_list(arg) && (compare_node(
				arg, car(t), FALSE) == 0))) {
			fp = (FILE *) t->n_obj;
			if (remove) {
				t->n_obj = NIL;
...
NODE *lclose(NODE *arg) {
	...
	if ((tmp = find_file(car(arg), TRUE)) == NULL)
		...
		margs = cons(caar(arg), cons(make_strnode((char *) tmp, NULL, strlen(
				(char *) tmp), STRING, strnzcpy), NIL));
		lmake(margs);
		free((char *) tmp);
	} else
		fclose(tmp);
	if ((is_list(car(arg)) && car(arg) == writer_name) || (!is_list(car(arg))
			&& (compare_node(car(arg), writer_name, FALSE) == 0))) {
		writer_name = NIL;
		writestream = stdout;
	}
	if ((is_list(car(arg)) && car(arg) == reader_name) || (!is_list(car(arg))
	...
	if (writestream == NULL) {
		...
		*print_stringptr = '\0';
		...
		if (find_file(writer_name, FALSE) == NULL) {
			...
			margs = cons(car(writer_name), cons(make_strnode(write_buf, NULL,
					strlen(write_buf), STRING, strnzcpy), NIL));
			lmake(margs);
			free(write_buf);
		}
		writer_name = NIL;
...
void silent_load(NODE *arg, char *prefix) {
	...
	if (prefix == NULL && arg == NIL)
		return;
	strcpy(load_path, (prefix == NULL ? "" : (arg == NIL ? prefix : addsep(
			prefix))));
	if (arg != NIL) {
		arg = cnv_node_to_strnode(arg);
		if (arg == UNBOUND)
			return;
		if (!strncmp(getstrptr(arg), ".", getstrlen(arg)))
			return;
		if (!strncmp(getstrptr(arg), "..", 2))
			return;
		if (getstrlen(arg) > 150)
			return;
	...
	if (loadstream != NULL) {
		...
		err_logo(CANT_OPEN_ERROR, make_strnode(load_path, NULL, strlen(
				load_path), STRING, strnzcpy));
